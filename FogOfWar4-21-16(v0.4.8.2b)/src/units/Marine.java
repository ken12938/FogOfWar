...
public class Marine extends Unit {
	...
	public Marine(int x, int y, boolean playerR, boolean playerB) {
		super(x, y, playerR, playerB);
		move_range = 150;
		projectile_range = LaserProjectile.getRange();
		vision_range = 100;
		vision_width = .8;
		...
		speed = .5;
		reload = 0;
		RELOAD = LaserProjectile.FIRE_RATE;
		burstFire = BURSTFIRE = 3;
		time = level.getTime();
	}
	...
	protected void shoot(double x, double y, double dir) {
		shotFired = true;
		Projectile p = new LaserProjectile(x - 8, y - 5, dir, UNIT_R, UNIT_G); 
